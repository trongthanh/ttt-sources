...
package  {
	...
	public class Demo4BoxTwoDRevolute extends Sprite {
		...
		public function Demo4BoxTwoDRevolute () {
			stage.frameRate = 60;
			...
			var gravity:b2Vec2 = new b2Vec2(0.0, 0.5);
			...
			boxShape = new b2PolygonShape();
			boxShape.SetAsBox(toPhysWorld(10), toPhysWorld(3));
			fixtureDef = new b2FixtureDef();
			fixtureDef.shape = boxShape;
			...
			for (i = 0; i < 11; i ++) {
				...
				//define joint
				if (i > 0) {
					jointDef.Initialize(prevBody, body, new b2Vec2(toPhysWorld(550 / 2 + i * space - 8), 11));
					world.CreateJoint(jointDef);
				} else {
					var distanceJoint: b2DistanceJointDef = new b2DistanceJointDef();
					distanceJoint.Initialize(prevBody, body, new b2Vec2(toPhysWorld(550 / 2), 11), new b2Vec2(toPhysWorld(550 / 2), 11));
		...
		private function render(event:Event):void {
			...
			lastNode.SetPosition(new b2Vec2(275 /scale + Math.cos(currentAngle * (Math.PI / 180)) * 6.2,
											11 + Math.sin(currentAngle * (Math.PI / 180)) * 6.2)
								);
			lastNode.SetAngle(currentAngle * (Math.PI / 180));
