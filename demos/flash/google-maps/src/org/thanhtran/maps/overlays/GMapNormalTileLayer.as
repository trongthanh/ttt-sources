...
package org.thanhtran.maps.overlays {
	...
	* @author Thanh Tran
	*/
	public class GMapNormalTileLayer extends TileLayerBase implements ITileLayer {
		...
		public function GMapNormalTileLayer(alpha: Number = 1) {
		...
		protected function gmapReadyHandler(event: Event): void {
			if (gmapInfo.isReady) {
				...
				while (tileLoader = pendingTileLoaders.pop()) {
					var tileUrl: String = gmapInfo.getNormalTileURL(tileLoader.tilePosition, tileLoader.zoom);
		...
		override public function loadTile(tilePosition: Point, zoom: Number): DisplayObject {
			...
			if (gmapInfo.isReady) {
				var tileUrl: String = gmapInfo.getNormalTileURL(tilePosition, zoom);
